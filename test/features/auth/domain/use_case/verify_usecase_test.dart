...
void main() {
  ...
  group('VerifyEmailUsecase Tests', () {
    test('Returns Failure when email is empty', () async {
      // Arrange
      var emptyEmailParams = VerifyEmailParams(
        email: "",
        otp: "123456",
      );
    ...
    test('Returns Failure when OTP is empty', () async {
      // Arrange
      var emptyOtpParams =
          VerifyEmailParams(email: "test@example.com", otp: "");
      when(() => repository.verifyEmail(any(), any())).thenAnswer(
          (_) async => const Left(ApiFailure(message: "OTP cannot be empty")));
